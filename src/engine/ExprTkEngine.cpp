...
bool ExprTkEngine::compile(const std::string& scriptUtf8)
{
    ...
    if (!parseRes.success) {
        // Parser-level error (unmatched (?=, empty expression). Keep
        // this state cleared - any subsequent execute() will surface
        // the failure via FormulaResult.success=false.
        return false;
    }
    ...
    for (std::size_t i = 0; i < parseRes.segments.size(); ++i) {
        ...
        if (!_parser.compile(seg.text, expr)) {
            // ExprTk failed to compile this expression. Bail out -
            // we don't want to leave a partially populated cache.
            _compiledExpressions.clear();
            return false;
        }
    ...
    int  /*documentCodepage*/)
{
    ...
    // changed since the last compile, run it now. Mirrors the Lua
    // bridge's ensureCompiled-on-first-execute pattern.
    if (!_haveCompiled || scriptUtf8 != _lastCompiledScript) {
        if (!compile(scriptUtf8)) {
            result.success      = false;
            result.errorMessage = !_parser.error().empty()
                ? _parser.error()
                : "ExprTk: failed to compile template";
            return result;
        }
    }
