...
{ L"msgbox_recoverable_runtime_error_details", L"Expression \"$REPLACE_STRING\" produced NaN.\n\nEither a string variable (FPATH, FNAME) was used where a number is expected, or a capture (hit, reg(N)) could not be parsed as a number.\n\nUse rstr(N) for the raw text, or guard with a NaN check (x != x)." },
{ L"msgbox_btn_skip_this_match", L"Skip this match" },
{ L"msgbox_btn_skip_all_nan", L"Skip all NaN" },
{ L"msgbox_btn_stop", L"Stop" },
{ L"status_exprtk_nan_skipped_summary", L"$REPLACE_STRING match(es) skipped (NaN)." },
{ L"msgbox_title_exprtk_nan_skipped_notice", L"NaN Matches Skipped" },
{ L"msgbox_exprtk_nan_skipped_notice", L"$REPLACE_STRING match(es) were skipped because their expression evaluated to NaN.\n\nThe replacement text was left unchanged for those matches." },
