...
namespace Encoding {
    ...
    EncodingInfo detectEncoding(const char* data, size_t len, const DetectOptions& opt) {
        ...
        // UTF-16 LE without BOM: same pre-checks and probe as N++
        // (Utf8_16_Read::determineEncoding). BE without BOM is deliberately
        // NOT detected - the mirrored probe would be a measurably worse bet:
        // its gate is p[0] == 0, and a leading NUL is one of the strongest
        // BINARY signatures there is; IsTextUnicode is documented for LE and
        // unreliable via the REVERSE_* flags; and Windows-native UTF-16 is LE
        // everywhere, while BOM-less BE producers normally do write a BOM.
        // A miss costs hits in one rare file and stays visible in the scan
        // summary; a false positive would decode binary as text (and round-
        // trip it in Replace in Files). N++'s own BE branch is commented out
        // citing weak detection. If a real case ever surfaces: use a strict
        // structural test (even length, even-indexed NULs), not IsTextUnicode.
