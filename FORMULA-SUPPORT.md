...
| Function          | Default `p` | Default `v` | Description                                                                 |
|-------------------|-------------|-------------|-----------------------------------------------------------------------------|
| `numout(N)`       | **0**       | `NaN`       | Block N's numeric output from the current match (earlier finished blocks). N is 1-based: `numout(1)` is the first block. |
...
| `txtout(N)`       | **0**       | `""`        | Block N's string output from the current match (earlier finished blocks). N is 1-based. |
| `txtout(N, P)`    | —           | `""`        | Block N's string output from match P ago.                                   |
| `txtout(N, P, V)` | —           | `V`         | Same, with explicit fallback string.                                        |

The block index `N` is **1-based**: `numout(1)` is the first block, `numout(2)` the second. `numout(0)` is invalid and returns the fallback (there is no "block zero"; this differs from `num(0)`, which mirrors the regex whole-match group). The arity-1 default `P=0` reads within the **current** match — the typical question is "what did an earlier block on this line produce?". A block can only read **earlier** finished blocks; reading the current block or any later one returns the fallback. Use an explicit `P>=1` to reach back into previous matches.

| Find            | Replace                                              | Description                                                                |
|-----------------|------------------------------------------------------|----------------------------------------------------------------------------|
| `^(\w+),(\d+)$` | `(?=num(2)) (?=numout(1) * 2)`                       | The second block reads the first block's output of the same match (default `P=0`) and doubles it. |
| `(\d+)`         | `(?=num(1)) prev:(?=numout(1, 1, 0))`                | First spec emits the value; second spec emits the previous match's first block (or 0 the first time). |
...
The first block passes the raw value through; the second block subtracts the raw
value of the *previous* line via `numout(1, 1, 0)` — first block, one match back,
fallback 0. This is the line-to-line delta.

- **Find:** `(\d+)`
- **Replace:** `(?=num(1)) (delta (?=num(1) - numout(1, 1, 0)))`

| Input line | Block 1 `num(1)` | `numout(1, 1, 0)` (prev. raw) | Block 2 delta | Output |
|------------|------------------|-------------------------------|---------------|--------|
...
The `1` in the middle of `numout(1, 1, 0)` is the lookback: one match back.
Without it, `numout(1)` would read the first block of the *same* match (the
default `P=0`) — here you explicitly want the previous line, so `P=1` is required.
The trailing `0` is the fallback for the first line, which has no predecessor.

**Example 2 — `numout` and `numprev` together (running total).**
The first block is the line subtotal again; the second block adds the subtotal of
*this* line (`numout(1)`, sideways) to the running total carried from the
*previous* line (`numprev()`, backwards in time).

- **Find:** `^(\d+),(\d+)$`
- **Replace:** `(?=num(1)*num(2)) running: (?=numout(1)+numprev())`

| Input line | Block 1 | `numout(1)` (this line) | `numprev()` (prev. total) | Block 2 |
|------------|---------|-------------------------|---------------------------|---------|
...
the chain starts cleanly at 20 instead of propagating `NaN`. `numout(1)` always
points at this line's own first block; `numprev()` always points at this block's
own output one match back.
...
| `^(\w+),(\d+)$` | `(?=num(2)) (?=numout(1, 0) * 2)`                    | The second block reads the first block's output within the same match and doubles it. `numout(1)` is equivalent, since `P=0` is the default. |
